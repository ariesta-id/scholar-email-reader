...
def process_batch(batch, title_col, abstract_col):
    ...
    return pd.DataFrame(results, columns=['relevance', 'probability'])
...
def process_excel_articles(input_path, batch_size=50):
    ...
    try:
        ...
        df = df.assign(relevance=pd.NA, probability=pd.NA)
        total = len(df)
        ...
        for start in range(0, total, batch_size):
            ...
            try:
                ...
                df.loc[start:end-1, ['relevance', 'probability']] = batch_results.values
